...
public class OrbXVlessQueryResolver {
    ...
    @Secured(USER)
    @QueryMapping
    public Map<String, Object> getOrbXVlessConfig(@Argument Long orbxServerId) {
        User currentUser = userService.getUser();
        log.info("User {} requesting OrbX VLESS config for server {}",
                currentUser.getEmail(), orbxServerId);

        OrbXVlessConfig config = vlessService.getOrCreateConfig(currentUser, orbxServerId);
    ...
    public List<Map<String, Object>> myOrbXVlessConfigs() {
        User currentUser = userService.getUser();
        List<OrbXVlessConfig> configs = vlessService.getUserConfigs(currentUser);
    ...
    @QueryMapping
    @Transactional(readOnly = true)
    public List<Map<String, Object>> myActiveOrbXVlessConfigs() {
        User currentUser = userService.getUser();
        List<OrbXVlessConfig> configs = vlessService.getActiveUserConfigs(currentUser);
