...
import com.orbvpn.api.repository.OrbXVlessConfigRepository;
...
public class OrbXVlessService {

    private final OrbXVlessConfigRepository configRepository;
     ...
     */
    @Transactional
    public OrbXVlessConfig getOrCreateConfig(User user, Long orbxServerId) {
        ...
        Optional<OrbXVlessConfig> existing = configRepository
                .findByUserUuidAndServer(user.getUuid(), server);

        if (existing.isPresent()) {
            OrbXVlessConfig config = existing.get();
        ...
        OrbXVlessConfig config = OrbXVlessConfig.builder()
        ...
        // Notify OrbX server to add user
        notifyServerAddUser(server, config);
    ...
    /**
     * Notify the OrbX server to add a VLESS user
     */
    private void notifyServerAddUser(OrbXServer server, OrbXVlessConfig config) {
        try {
            ...
            log.error("❌ Error notifying OrbX server to add VLESS user: {}", e.getMessage());
    ...
    public boolean revokeConfig(User user, Long orbxServerId) {
        ...
        Optional<OrbXVlessConfig> config = configRepository
                .findByUserUuidAndServer(user.getUuid(), server);
        ...
        OrbXVlessConfig vlessConfig = config.get();
    ...
    /**
     * Notify the OrbX server to remove a VLESS user
     */
    private void notifyServerRemoveUser(OrbXServer server, OrbXVlessConfig config) {
        try {
            String endpoint = getServerEndpoint(server);
            ...
            log.info("✅ Notified OrbX server {} to remove VLESS user {}",
    ...
    public List<OrbXVlessConfig> getUserConfigs(User user) {
     ...
     */
    @Transactional(readOnly = true)
    public List<OrbXVlessConfig> getActiveUserConfigs(User user) {
     ...
     */
    @Transactional
    public OrbXVlessConfig syncConfig(User user, Long orbxServerId, String vlessUuid,
     ...
     */
    @Transactional(readOnly = true)
    public Optional<OrbXVlessConfig> getConfig(User user, Long orbxServerId) {
        return configRepository.findByUserUuidAndServerId(user.getUuid(), orbxServerId);
    }
