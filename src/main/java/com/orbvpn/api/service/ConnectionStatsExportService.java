...
public class ConnectionStatsExportService {
    ...
    private byte[] generateDetailedCsv(List<ConnectionStats> stats) {
        try (ByteArrayOutputStream out = new ByteArrayOutputStream();
                ...
                CSVPrinter csvPrinter = new CSVPrinter(writer, CSVFormat.DEFAULT.withHeader(DETAILED_HEADERS))) {
    ...
    private byte[] generateAggregateCsv(List<ConnectionStatsAggregate> aggregates) {
        try (ByteArrayOutputStream out = new ByteArrayOutputStream();
                OutputStreamWriter writer = new OutputStreamWriter(out, StandardCharsets.UTF_8);
                CSVPrinter csvPrinter = new CSVPrinter(writer, CSVFormat.DEFAULT.withHeader(AGGREGATE_HEADERS))) {
