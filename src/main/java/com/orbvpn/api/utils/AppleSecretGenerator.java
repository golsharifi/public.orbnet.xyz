...
public class AppleSecretGenerator {
    ...
    private static final Logger log = LoggerFactory.getLogger(AppleSecretGenerator.class);

    public String createClientSecret() {
        var resource = new ClassPathResource("apple/AuthKey_B7P9ALAXDU.p8");
        try (
                InputStream inputStream = resource.getInputStream();
                StringReader reader = new StringReader(
                        new String(inputStream.readAllBytes(), StandardCharsets.UTF_8))) {

            log.info("Successfully accessed file from classpath: {}", resource.getPath());
            ...
            builder.signWith(privateKey)
                    ...
                    .and();
            String clientSecret = builder.compact();

            log.info("Successfully created new client secret: {}", clientSecret);
            return clientSecret;
        } catch (IOException e) {
            log.error("Error while creating client secret: {}", e.getMessage());
