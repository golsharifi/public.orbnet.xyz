...
import static com.orbvpn.api.domain.enums.RoleName.Constants.ORBX_SERVER;
...
public class RateLimitFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(jakarta.servlet.http.HttpServletRequest request,
        ...
        // ✅ NEW: Skip rate limiting for OrbX servers (server-to-server communication)
        Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
        if (authentication != null && authentication.getAuthorities() != null) {
            for (GrantedAuthority authority : authentication.getAuthorities()) {
                if (ORBX_SERVER.equals(authority.getAuthority())) {
                    // OrbX servers are trusted - skip all rate limiting
                    filterChain.doFilter(request, response);
                    return;
                }
            }
    ...
    private String retrieveUserIdFromRequest(HttpServletRequest request) {
        ...
        if (authentication.getPrincipal() instanceof UserDetails) {
            ...
            // Handle OrbX server authentication (principal is just a string)
        ...
        if (authentication != null && authentication.getAuthorities() != null) {
            for (GrantedAuthority authority : authentication.getAuthorities()) {
                if (ADMIN.equals(authority.getAuthority())
                        ...
                        || ORBX_SERVER.equals(authority.getAuthority())) {
